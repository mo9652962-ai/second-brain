...
L.append('')
L.append(f'> 自动生成：`scripts/gen-vault-index.py`（{now.strftime("%Y-%m-%d %H:%M")}）。')
...
D.append('')
D.append(f'> 自动生成：`scripts/gen-vault-index.py`（{now.strftime("%Y-%m-%d %H:%M")}）。详细报告：`scripts/vault-audit-report.json`（CI 周一产物）。')
D.append('')
D.append('## 关键指标')
D.append('')
