...
ENV_FILE="%USERPROFILE%/AppData/Local/hermes/.env"
set -a
source "$ENV_FILE"
set +a
...
ARK_KEY=$(python -c "import yaml;print(yaml.safe_load(open(r'%USERPROFILE%/AppData/Local/hermes/config.yaml',encoding='utf-8'))['custom_providers'][3]['api_key'])" 2>/dev/null)
...
KIMI_KEY=$(python -c "import yaml;print(yaml.safe_load(open(r'%USERPROFILE%/AppData/Local/hermes/config.yaml',encoding='utf-8'))['providers']['moonshot']['api_key'])" 2>/dev/null)
if [ -n "$KIMI_KEY" ]; then
