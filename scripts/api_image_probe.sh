...
set -a
source "%USERPROFILE%/AppData/Local/hermes/.env"
set +a

TODAY=$(date +%Y-%m-%d)
MONTH=$(date +%m)
YEAR=$(date +%Y)
OUT="%USERPROFILE%/.openclaw/workspace/memory/${YEAR}/${MONTH}/${TODAY}-api-probe.md"
mkdir -p "%USERPROFILE%/.openclaw/workspace/memory/${YEAR}/${MONTH}"
