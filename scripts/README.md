...
> **校验规则（2026-09-04）**：生成类脚本（出图/出视频/出文档）必须输出确定性校验命令（stat/读文件头/尺寸），并登记在本表用途列——视觉模型仅辅助审美，不当规格断言（9/3 闲鱼主图教训）。
>
...
| assert_state_consistency.py | projects/state.yaml 与 current.md「第 N 天」一致性断言门禁（只读；跨 cron 计数收敛的写后校验，输出 PASS/FAIL；9/10 反思建，建库即抓出 40 vs 41 漂移残留）。9/21 扩展：新增 reflection/daily-review 表格行动项行扫描（行首\|+闲鱼+第N天，>=state.yaml updated_at 日期文件），叙述/机制引用不算漂移 | 09-10 | 在用 |
