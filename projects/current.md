...
### 🧭 9/25 反思行动项（daily-todo-executor 复盘 9-25，执行者必读）

- [x] 🔴 摄入注入防线扩面（按 arXiv 2609.29775「推理段 + 输出前缀」组合式注入）→ ✅ 2026-09-25 daily-todo-executor 落地：`ingest_injection_scan.py` 从 4 类条件式模式扩到 **13 条模式**（新增第 5 组「组合式前缀注入」6 条：推理段↔前缀 四向组合 + 推理段→动作→高危对象 中英两条；新增 REASON_EN/ZH + PREFIX_EN/ZH 词表）。自检 **8 正样本全中 / 7 负样本零误报**（新增 3 条「推理词单独出现」负样本，防论文正文误报）；命令式基线对比 2/8（复现论文「条件式/组合式可绕过传统检测器」论点）；真实语料实测（9/24–9/25 arXiv 速览 + HN + 近月 knowledge 60 文件）仅 7 处命中且均为叙述性文字，无爆炸
...
_由 k (Hermes) 在每次会话结束时更新 | 最后更新: 2026-09-25 (daily-todo-executor 9/25：落地组合式注入防线扩面 + 技能范围声明试点 + 轨迹哈希账本 cron + 万悟截止处置；闲鱼权威第 42 天未变)_
