...
- [x] 闲鱼上架素材包预生成（knowledge/闲鱼上架素材包-预生成.md）✅ 7/30
...
### 🎯 闲鱼上架（P0，8/2 执行，原 8/1 解封日已过）
- [ ] 上架「AI 代做 PPT」商品（素材包已就绪，30min）→ **排期 8/2**，待 sora 操作
- [ ] 主图制作：3 张模板图（前后对比/价格表/服务承诺）+ 样例截图打水印 → 排期 8/2
- [ ] 同步上架「论文排版/润色」商品（素材包已有现成文案）→ 排期 8/2 同批上
- [ ] 补 PPT 样例素材：从现有作品提 2-3 个样例页 + 「仅供参考」水印 → portfolio/ → 排期 8/2
- [ ] 数学练习册定制文案挂载（35元/份）→ 排期 8/2 顺带

### 📱 内容引流（P1，依赖 PPT 样例）
- [ ] 小红书发「AI PPT 教程」内容（可复用 PPT 样例）→ 排期 8/3（PPT 样例完成后）
- [ ] 尝试接论文润色/翻译单（依赖商品上架后引流）→ 排期 8/4 起观察

### 🛠️ 工具/知识侧（P2，可选）
- [ ] Krea2 安装（ComfyUI 搭建 + 14GB int8-convrot 模型下载）→ 排期 8/3+，需安排下载时间
...
| 项 | 状态 | 说明 |
|:---|:-----|:-----|
| 随身WiFi下单（赫电 Pro 399元/年） | 🔒 选型已确认 | 33元/月 1500G，待确认下单 |
...
_由 k (Hermes) 在每次会话结束时更新 | 最后更新: 2026-08-01（周度清理）_
