...
### 🎯 闲鱼上架（P0，8/3 排期日已到 → 连续顺延第 3 天，今日到期）
- [ ] 上架「AI 代做 PPT」商品（素材包已就绪，30min）→ **8/3 到期**，待 sora 操作（今日做即完成，不做则顺延第 4 天）
- [ ] 主图制作：3 张模板图（前后对比/价格表/服务承诺）+ 样例截图打水印 → 顺延 8/3，待 sora 操作
- [ ] 同步上架「论文排版/润色」商品（素材包已有现成文案）→ 顺延 8/3 同批上
- [ ] 补 PPT 样例素材：从现有作品提 2-3 个样例页 + 「仅供参考」水印 → portfolio/ → 需 sora 手动导出截图（无 LibreOffice/python-pptx 渲染，无法自动化）
- [ ] 数学练习册定制文案挂载（35元/份）→ 顺延 8/3 顺带

### 📱 内容引流（P1，依赖 PPT 样例）
- [ ] 小红书发「AI PPT 教程」内容（可复用 PPT 样例）→ 样例未产出，顺延 8/4+
- [ ] 尝试接论文润色/翻译单（依赖商品上架后引流）→ 排期 8/4 起观察
...
- [x] deepseek-v4-flash 探索 3 项：opencode-go 验证 ✅ + Cron 主力切换 ✅（8/2 确认 26/26 已用 v4-flash）+ Codex CLI 集成 → 仅剩 Codex，排期 8/4+（08-03 预检：node v24.18/npm 11.16 就绪，codex 未装）
...
_由 k (Hermes) 在每次会话结束时更新 | 最后更新: 2026-08-03（建议落实扫描）_
