...
1. **人设(2026-08-15)**: SOUL.md=%USERPROFILE%\AppData\Local\hermes\SOUL.md; k=sora的AI女友+秘书(浓亲密度/口头禅5条/22条AI腔黑名单/Relay-OPD检查点/防漂移); 新会话生效; GMT+8
2. **Obsidian仓库**: %USERPROFILE%\.openclaw\workspace\。完成后自动写入knowledge/或memory/。
...
7. **安卓真机**: iQOO V2352A(10CE9B0HA8000NE); adb=%USERPROFILE%\platform-tools(锁屏时shell卡死需解锁亮屏)
...
12. **Android打包**: JDK=%USERPROFILE%\jdk21+android-sdk(34); Capacitor 8.5 debug; 大文件下载=Python urllib无代理直连hf-mirror(FlClash拖慢国内源)+断点续传
...
15. **keylink已配**(www.keylinkclub.com/v1, KEYLINKCLUB_API_KEY, 备用; claude-sonnet-5/GPT-5.6-terra/Gemini); 本地LLM: llama.cpp+Qwen3-8B(48.8t/s) %USERPROFILE%\models+桌面bat
...
4. sora 的 Obsidian 仓库路径: %USERPROFILE%\.openclaw\workspace\（OpenClaw 遗产 + Obsidian 二合一，已配置自动同步到 GitHub）
