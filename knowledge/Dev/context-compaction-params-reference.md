...
- [x] 会话 >80% 上下文占用时 → 主动触发压缩/归档（比例触发）（参考清单，由 Hermes 自动管理）
- [x] 归档保留尾部 20K tokens（最近对话优先）（参考清单，由 Hermes 自动管理）
- [x] 小会话（<2K tokens）不做压缩开销（参考清单，由 Hermes 自动管理）
- [x] 压缩动作记录到日志（可审计）（参考清单，由 Hermes 自动管理）
