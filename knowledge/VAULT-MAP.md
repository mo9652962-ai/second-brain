---
title: VAULT-MAP
aliases: [VaultMap, 机器索引, vault-map]
type: meta
domain: META
status: active
created: 2026-09-21
updated: 2026-09-21
tags: [meta/index, knowledge/governance, ai/navigation]
---

# 🗺️ VAULT-MAP — 机器可读索引

> 自动生成：`scripts/gen-vault-index.py`（2026-09-21 13:48）。
> 给 AI Agent 用的静态导航索引：不用全盘扫描即可定位内容。人类入口见 [[knowledge-map]] 与 [[HOME]]。

## 顶层分布

| 顶层 | md 数 |
|:---|---:|
| `knowledge/` | 678 |
| `memory/` | 318 |
| `skills/` | 131 |
| `projects/` | 10 |
| `templates/` | 7 |
| `portfolio/` | 5 |
| `pipelines/` | 4 |
| `outputs/` | 3 |
| `playbooks/` | 3 |
| `concepts/` | 2 |
| `docs/` | 2 |
| `health/` | 2 |
| `AGENTS.md/` | 1 |
| `CHANGELOG.md/` | 1 |
| `CODE_OF_CONDUCT.md/` | 1 |
| `CONTRIBUTING.md/` | 1 |
| `DREAMS.md/` | 1 |
| `HEARTBEAT.md/` | 1 |
| `HOME.md/` | 1 |
| `IDENTITY.md/` | 1 |
| `INDEX.md/` | 1 |
| `MEMORY.md/` | 1 |
| `README.md/` | 1 |
| `SECURITY.md/` | 1 |
| `SOUL.md/` | 1 |
| `SUPPORT.md/` | 1 |
| `TOOLS.md/` | 1 |
| `USER.md/` | 1 |
| `graphify-out/` | 1 |
| `META/` | 1 |
| `scripts/` | 1 |
| `system/` | 1 |
| `todo/` | 1 |
| `traces/` | 1 |

## 知识域分布（knowledge/）

| 域 | 笔记数 |
|:---|---:|
| Research | 235 |
| Dev | 143 |
| Productivity | 54 |
| Security | 53 |
| cards | 37 |
| Daily | 34 |
| Hardware | 21 |
| AI | 19 |
| Finance | 16 |
| Archive | 13 |
| Content | 10 |
| SOP | 10 |
| META | 9 |
| knowledge | 8 |
| Creative | 6 |
| gaming | 4 |
| Education | 2 |
| Product | 2 |
| Projects | 2 |

## MOC 锚点

- [[knowledge/AI/MOC-AI]]
- [[knowledge/Archive/MOC-Archive]]
- [[knowledge/cards/MOC-cards]]
- [[knowledge/Content/MOC-Content]]
- [[knowledge/Creative/MOC-Creative]]
- [[knowledge/Daily/MOC-Daily]]
- [[knowledge/Dev/MOC-Dev]]
- [[knowledge/Education/MOC-Education]]
- [[knowledge/Finance/MOC-Finance]]
- [[knowledge/gaming/MOC-gaming]]
- [[knowledge/Hardware/MOC-Hardware]]
- [[knowledge/META/MOC-META]]
- [[knowledge/MOC-Duplicate-Review]]
- [[knowledge/MOC-Inbox]]
- [[knowledge/Product/MOC-Product]]
- [[knowledge/Productivity/MOC-Productivity]]
- [[knowledge/Projects/MOC-Projects]]
- [[knowledge/Research/MOC-GitHub]]
- [[knowledge/Research/MOC-Research]]
- [[knowledge/Security/MOC-Security]]
- [[knowledge/SOP/MOC-SOP]]

## 导航规则

- 知识笔记 → `knowledge/<域>/`；领域总览 → 对应 `MOC-*`
- 记忆/日记/反思 → `memory/`（入口 MOC-Memory，见 memory 目录）
- 可执行技能 → `skills/`；项目产物 → `projects/`、`outputs/`
- 回答前先读 [[knowledge-map]]（人类地图）与本页（机器地图）
